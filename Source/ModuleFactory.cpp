...
ModuleFactory::ModuleFactory()
{
   ...
   REGISTER_HIDDEN(VSTPlugin, vstplugin, kModuleCategory_Synth);
...
std::vector<std::string> ModuleFactory::GetSpawnableModules(ModuleCategory moduleType)
{
...
std::vector<std::string> ModuleFactory::GetSpawnableModules(std::string keys, bool continuousString)
{
   ...
   for (auto vstFile : vsts)
   {
      ...
      if (CheckHeldKeysMatch(vstName, keys, continuousString))
   ...
   {
      if (CheckHeldKeysMatch(prefab, keys, continuousString) || keys[0] == ';')
