...
void TapTempo::DrawModule()
{
   ...
   if (mNumAccumulatedSamples > 1)
   {
      ...
      if (mRoundTempo)
      {
         float roundedTempo = round(mLastCalculatedTempo);
         float diff = mLastCalculatedTempo - roundedTempo;
         DrawTextNormal("tempo: " + ofToString(roundedTempo, 2) + " (" + ofToString(diff, 2) + ")", 5, y);
      }
      else
      {
         DrawTextNormal("tempo: " + ofToString(mLastCalculatedTempo, 2), 5, y);
      }
      DrawTextNormal("std dev: " + ofToString(mStandardDeviation, 2), 5, y + 14);
   }
}
...
void TapTempo::ButtonClicked(ClickButton* button, double time)
{
   if (button == mTapButton)
   {
      if (mNumAccumulatedSamples > 0)
      {
         ...
         const float kRestartThresholdMs = 2000; // 30 bpm
   ...
   {
      if (mRoundTempo)
         TheTransport->SetTempo(round(mLastCalculatedTempo));
      else
         TheTransport->SetTempo(mLastCalculatedTempo);
